...
enum { COUNTER_LIMIT = 64, NUM_THREADS = 2, SLEEP_INTERVAL = 1 };
...
static int __increment_counter(void *arg)
{
	...
	while (this_cpu_read(counter) < COUNTER_LIMIT) {
		...
		pr_info(KBUILD_MODNAME
			...
			tdata->id, this_cpu_read(counter));
...
MODULE_DESCRIPTION(
	KBUILD_MODNAME
	" - A Linux kernel module that demonstrates safe handling of data among kernel threads.");
